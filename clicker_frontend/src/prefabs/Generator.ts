...
export default class Generator {
  ...
  private backgroundTileName: string | Phaser.Textures.Texture =
    "backgroundDirt";
  ...
  createSupportBars(x: number, y: number, counter: number) {
    ...
    let bars = [];
    leftSpr = this.scene.add.sprite(
      this.layers.sideFloor[x][3].x,
      this.layers.sideFloor[y][4].y,
      "barLeft"
    );
    leftSpr.setOrigin(0);
    leftSpr.setDepth(this.PRIORITY.objects);

    rightSpr = this.scene.add.sprite(
      this.layers.sideFloor[x][12].x,
      this.layers.sideFloor[y][4].y,
      "barRight"
    );
  ...
  createDrill() {
    // Create Drill
    this.drill = this.scene.physics.add.sprite(
      700,
      this.miner.y,
      "drillAnimation"
    );
    ...
    this.scene.anims.create({
      key: "drill",
      frames: this.scene.anims.generateFrameNumbers("drillAnimation", {
        start: 0,
        end: 1,
      }),
      frameRate: 6,
  ...
  setupMinerAnimations() {
    ...
    this.scene.anims.create({
      key: "mining",
      frames: this.scene.anims.generateFrameNumbers("miningAnimation", {
        start: 0,
        end: 4,
      }),
      frameRate: 15,
    ...
    this.scene.anims.create({
      key: "idle",
      frames: this.scene.anims.generateFrameNumbers("idleAnimation", {
        start: 0,
        end: 7,
      }),
      frameRate: 6,
  ...
  scrollBackGround() {
    let offset =
      this.scene.cameras.main.scrollY - this.layers.background[0][0].y;
  ...
  scrollSideFloor() {
    let offset =
      this.scene.cameras.main.scrollY - this.layers.sideFloor[0][0].y;
  ...
  destroyFloor() {
    for (let tx = 4; tx < 12; tx++) {
      this.debris.emitParticleAt(
        this.layers.floor[0][tx].x + this.CONFIG.tile / 2,
        this.layers.floor[0][tx].y,
        3
      );
  ...
  destroyPickups() {
    this.layers.pickups.forEach((sprite) => {
      if (
        sprite.getBounds().y - this.scene.cameras.main.scrollY <=
        -this.CONFIG.tile
      ) {
        sprite.destroy();
      }
  ...
  appendPickups(x: number, y: number) {
    ...
    if (this.pickedFirstDiamond) {
      this.randomDiamond =
        Math.floor(Math.random() * (randomDiamond - 1 + 1)) + 1;
    }
    if (this.scene.depth >= 500 && this.randomDiamond == 1) {
      ...
      spr.setInteractive({
        cursor: "url(src/assets/img/hand_cursor_small.png), pointer",
      });
      spr.on("pointerdown", (event: any) => {
        ...
        this.layers.pickups.forEach((sprite) => {
          if (
            sprite
              .getBounds()
              .contains(pointer.x, pointer.y + this.scene.cameras.main.scrollY)
          ) {
            sprite.destroy();
          }
        });
      ...
      Math.floor(Math.random() * (randomTreasure - 1 + 1)) + 1 ===
      10
    ) {
  ...
  crackFloorRow() {
    ...
    for (let tx = 4; tx < 12; tx++) {
      switch (this.breakCounter) {
        case 0:
          spr = this.scene.physics.add.staticSprite(
            this.layers.floor[0][tx].x,
            this.layers.floor[0][tx].y,
            this.crackedTileName + "1"
          );
          break;
        case 1:
          spr = this.scene.physics.add.staticSprite(
  ...
  loadGameData() {
    ...
    if (this.scene.loggedIn) {
      ...
      this.barRowCounter = Math.floor(
        this.scene.depth / 5 < 10 ? this.scene.depth / 5 : 10
      );
